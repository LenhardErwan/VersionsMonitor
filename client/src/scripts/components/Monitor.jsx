    ...
    return (
      <article className="monitor uk-animation-slide-bottom-medium uk-card uk-card-default uk-card-body">
        <img src={this.state.params.url === this.state.params.image_url ? `${new URL(this.state.params.url).origin}/favicon.ico` : this.state.params.image_url} />
				...
				<div><a href={this.state.params.url} uk-tooltip={`title: Open the following link in a new tab: ${this.state.params.url}`} onClick={(event) => {event.preventDefault(); const win = window.open(this.state.params.url, '_blank'); win.focus();}}>Link</a> (open in new tab)</div>
